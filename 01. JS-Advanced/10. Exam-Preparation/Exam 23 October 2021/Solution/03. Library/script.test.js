let library = require('./script.js');
let assert = require('chai').assert;

describe('library', () => {
    describe('calcPriceOfBook', () => {
        it('Should throw error if name of book is not a string', () => {
            assert.throw(() => { library.calcPriceOfBook(2, 2000) }, Error, 'Invalid input');

            assert.throw(() => { library.calcPriceOfBook(true, 2000) }, Error, 'Invalid input');

            assert.throw(() => { library.calcPriceOfBook(undefined, 2000) }, Error, 'Invalid input');
        });

        it('Should throw error if price of book is not a intiger number', () => {
            assert.throw(() => { library.calcPriceOfBook('Limitless', 2.2) });

            assert.throw(() => { library.calcPriceOfBook('Mecho pux', undefined) });

            assert.throw(() => { library.calcPriceOfBook('Think and grow', '2') });
        });
    ...
    describe('findBook', () => {
        it('Should throw Error if bookArray is empty', () => {
            assert.throw(() => { library.findBook([], 'Limitless') });
        });
    ...
    describe('arrangeTheBooks', () => {
        it('Should throw error if book count is not a integer number or is negative number', () => {
            assert.throw(() => { library.arrangeTheBooks(2.2) });

            assert.throw(() => { library.arrangeTheBooks('2') });

            assert.throw(() => { library.arrangeTheBooks(undefined) });

            assert.throw(() => { library.arrangeTheBooks(-1) });

            assert.throw(() => { library.arrangeTheBooks(-10) });
        });
